...
from energy import EnergyApp
from utils import Reshetka, Line


class Processy():
    def __init__(self, resh, radio_check, a, b, window, graph=None, ):
        """
...
class View():
    def __init__(self):
        ...
        self.btn_start.clicked.connect(self.open_drawer)
        ...
        self.images = [QtGui.QPixmap('images\треугольник'), QtGui.QPixmap('images\квадрат.png'),
        ...
        for i in range(self.a):
            for j in range(self.b):
                ...
                self.resh.matrix[i][j].setBrush(
                    QtGui.QBrush(self.resh.matrix[i][j].color, style=QtCore.Qt.SolidPattern))
